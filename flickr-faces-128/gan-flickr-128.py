...
LAMBDA_GP = 10.0
BETA_1 = 0.0  # changed from 0.1 --> 0.0 at epoch 78
...
if initial_epoch == 0:
    ...
    gen_model = tf.keras.Sequential([
        tf.keras.layers.Reshape((1, 1, Z_SIZE), input_shape=(Z_SIZE,)),
        tf.keras.layers.Conv2DTranspose(filters=Z_SIZE, kernel_size=4, strides=1, padding='valid', use_bias=False),
    ...
    disc_model = tf.keras.Sequential([
        tf.keras.layers.Conv2D(filters=FILTERS[GEN_DIM], kernel_size=KERNEL_SIZE, padding='same',
    ...
    disc_model.add(tf.keras.layers.Conv2D(filters=FILTERS[output_dim], kernel_size=4, padding='valid'))
    disc_model.add(tf.keras.layers.LayerNormalization())
    disc_model.add(tf.keras.layers.LeakyReLU())
    ...
    gen_model = tf.keras.models.load_model(os.path.join('objects', 'flickr-faces', f'gen_model_{initial_epoch:03d}.h5'))
    disc_model = tf.keras.models.load_model(
        os.path.join('objects', 'flickr-faces', f'disc_model_{initial_epoch:03d}.h5'))
...
for epoch in range(1, n_epochs + 1):
    ...
    gen_model.save(os.path.join('objects', 'flickr-faces', f'gen_model_{epoch + initial_epoch:03d}.h5'))
    disc_model.save(os.path.join('objects', 'flickr-faces', f'disc_model_{epoch + initial_epoch:03d}.h5'))
