...
if initial_epoch == 0:
    ...
    disc_model = tf.keras.models.load_model(os.path.join('objects', f'disc_model_{initial_epoch:03d}.h5'))

with open(os.path.join('logs', f'gen_model_summary-{initial_epoch:03d}.txt'), 'w') as f_gen_model_summary:
    gen_model.summary(print_fn=(lambda x: f_gen_model_summary.write(f'{x}\n')))

gen_model.summary()

with open(os.path.join('logs', f'disc_model_summary-{initial_epoch:03d}.txt'), 'w') as f_disc_model_summary:
...
plt.tight_layout()
plt.savefig(os.path.join('logs', f'learning_curve-{initial_epoch:03d}.png'))
...
plt.tight_layout()
plt.savefig(os.path.join('logs', f'generated_faces-{initial_epoch:03d}.png'))
